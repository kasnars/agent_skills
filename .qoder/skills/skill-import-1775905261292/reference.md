 ...
 */
async function publishToDraft(docxPath, title, author = '') {
    try {
...
if (require.main === module) {
    const [docxPath, title, author] = process.argv.slice(2);
    
    if (!docxPath || !title) {
        console.log('用法: node publish-draft.js <docx文件路径> <标题> [作者]');
        process.exit(1);
    }
    
    publishToDraft(docxPath, title, author).then(result => {
        if (result.success) {
            console.log('\n🎉 发布成功！');
